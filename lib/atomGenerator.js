...
`;
        xml += `    <updated>${link.pubDate ? new Date(link.pubDate).toISOString() || now : now}</updated>
`;
        xml += `    <summary type="text">${escapeXml(link.title)}${link.pubDate ? ` - ${link.pubDate}` : ''}</summary>
`;
        xml += `    <author>
`;
