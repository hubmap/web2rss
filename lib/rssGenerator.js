    ...
    try {
      ...
      parsedData.links.forEach((link, index) => {
        feed.item({
          ...
          description: escapeXml(`${link.title}${link.pubDate ? ` - ${link.pubDate}` : ''}`),
          url: link.url,
          guid: `${link.url}#${index}`,
          date: link.pubDate ? new Date(link.pubDate).toISOString() || new Date().toISOString() : new Date().toISOString(),
          author: 'Web2RSS'
        });
      });
